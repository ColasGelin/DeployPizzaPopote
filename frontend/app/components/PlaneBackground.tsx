...
const PlaneBackground: React.FC<PlaneBackgroundProps> = ({
  isActive,
  isInvisible = false,
  position = [-5, 2, 1],
  items
  ...
  if (camera instanceof PerspectiveCamera) {
    ...
    const distance = 1;
    planeHeight = 4.4 * Math.tan(fov / 2) * distance;
    planeWidth = 1;
  } else {
    planeHeight = 4.4;
    planeWidth = 1;
  ...
  if (!isActive || loadedTextures.length !== items.length) return null;

  return (
    <group
      position={new Vector3(...position)}
      rotation={[-0.5, Math.PI / 4.7, 0.33]}
      visible={opacity > 0}
      frustumCulled={false}
